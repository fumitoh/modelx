...
- Revision 2026-07-18 (post-completion cleanup #2): **`spmgr` slot removed from `Impl`** — redundant, since `impl.spmgr is impl.model.spmgr` always held (both threaded down the same parent chain at construction). `Impl.spmgr` is now a property reading `self.model.spmgr`; `ModelImpl` shadows it with a real slot that owns the `SpaceManager` instance (still pickled there — the model.py alias stays). Constructors lost the `spmgr=` parameter (base.py, model.py, space.py, cells.py, reference.py, macro.py). Legacy pickles carry a `spmgr` entry per Impl; tolerated by a new `Impl.__setstate__` (handles the default `(dictstate, slotstate)` shape; the `AttributeError` from `setattr` is swallowed only for the `spmgr` key — non-ModelImpl impls thereby discard it, while `ModelImpl` **stores** it into its own slot) and a `state.pop("spmgr")` in `BaseSpaceImpl.__setstate__`. The full-mixin variant (SpaceManager as a ModelImpl mixin base) was considered and declined: ModelImpl's global-ref `new_ref`/`change_ref`/`del_ref` trio collides with SpaceManager's space-level signatures (and `_check_sanity` would self-recurse), and old pickles' embedded SpaceManager instances would need a dict-ful legacy shim class.
